...
class DynamoDB:
    ...
    def return_df(self):
        ...
        print("DynamoDB Tables dataframe complete")
        if r_df.empty:
            print("DynamoDB Tables dataframe empty")
        return r_df
...
def test_run():
    for profile in boto3.session.Session().available_profiles:
        if profile == "default":
            ...
            test_run = DynamoDB(session)
            df = test_run.return_df()
            test_run.make_blank_excel(profile)
            test_run.write_new_sheet(profile, df, "DynamoDB")

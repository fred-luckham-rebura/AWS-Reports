...
class PatchGroups:
    ...
    def return_df(self):
        ...
        print("Patch Groups dataframe complete")
        if r_df.empty:
            print("Patch Groups dataframe empty")
        return r_df
    ...
    def filter_response(self, paginator, region):
        for page in paginator.paginate():
            for pg in page["Mappings"]:
                pg_dict = {
                    'Region':region,
                    'PatchGroup':jmespath.search("PatchGroup", pg),
                    'BaselineId':(str(jmespath.search("BaselineIdentity.BaselineId", pg)).strip("[]")),
                    'BaselineName':jmespath.search("BaselineIdentity.BaselineName", pg),
                    'OperatingSystem':jmespath.search("BaselineIdentity.OperatingSystem", pg),
                    'BaselineDescription':jmespath.search("BaselineIdentit.BaselineDescription", pg),
                    'DefaultBaseline':(str(jmespath.search("BaselineIdentity.DefaultBaseline", pg)).strip("[]")),
                    }
                if pg_dict:
                    self.r_list.append(pg_dict)
                else:
                    pass
## Write to excel
...
def test_run():
    for profile in boto3.session.Session().available_profiles:
        if profile == "default":
            ...
            test_run = PatchGroups(session)
            df = test_run.return_df()
            test_run.make_blank_excel(profile)
            test_run.write_new_sheet(profile, df, "PatchGroups")
